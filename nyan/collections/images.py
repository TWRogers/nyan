...
COLOUR_MODES = [('R', 'G', 'B'), ('B', 'G', 'R'), ('X', 'Y', 'Z'),
                ('Y', 'Cr', 'Cb'), ('L', 'a', 'b'), ('L', 'u', 'v'),
                ('H', 'L', 'S'), ('H', 'S', 'V'), ('GRAY', )]


class Images(object):
    def __init__(self, src, debug_mode: bool = False) -> None:
    ...
    def get_relative_coords(self, normalise=False):
        top_right = self.size
        bottom_left = (0, 0)
        relative_top_right = list(
            self.transform_point(top_right, 'end', 'start'))
        relative_bottom_left = list(
            self.transform_point(bottom_left, 'end', 'start'))
        if normalise:
    ...
    def convert_color(self, channel_mode: tuple):
        ...
        assert self.channel_mode != (
            'GRAY', ), 'Cannot convert from gray to a colour space.'
        conversion_str = "COLOR_{}2{}".format(''.join(self.channel_mode),
                                              ''.join(channel_mode))
        assert hasattr(cv2, conversion_str), \
            '{} does not appear to be a valid cv2 colour conversion'.format(conversion_str)
        self.images = [
            cv2.cvtColor(image, getattr(cv2, conversion_str))
    ...
    def select_channel(self, channel: str = 'R'):
        ...
        self.images = [
            _repeat_c_channels(image[..., channel_index])
            for image in self.images
        ]
    ...
    def from_array(cls,
        ...
        if array.ndim == 4:
            ...
            assert channel_mode == (
                'GRAY', ), "channel_mode must be ('GRAY',) if array.ndim == 2"
            images = [_repeat_c_channels(array)]
    ...
    def transform_image(self,
                        ...
                        end_event: str = 'end') -> np.ndarray:
        relevant_transform_history, direction = self._get_relevant_history(
        ...
        if direction:
            ...
            for event in relevant_transform_history[::-1]:
                try:
                    getattr(image, '{}_inverse'.format(
                        event['fn_name']))(*event['fn_args']['args'],
                                           **event['fn_args']['kwargs'])
                except AttributeError:
                    raise NotImplementedError(
                        'Inverse fn ({}_inverse) '
                        'for {} is not Implemented'.format(
                            event['fn_name'], event['fn_name']))
        return image

    def transform_point(self,
                        point,
                        start_event: str = 'start',
                        end_event: str = 'end') -> tuple:
        relevant_transform_history, direction = self._get_relevant_history(
            start_event, end_event)
        ...
        if direction:
            for event in relevant_transform_history:
                ...
                try:
                    point = getattr(self,
                                    fn_name)(point, *event['fn_args']['args'],
                                             **event['fn_args']['kwargs'])
                except AttributeError:
    ...
    def transform_box(self,
                      ...
                      start_event: str = 'start',
                      end_event: str = 'end') -> tuple:
        max_point = self.transform_point((box[1], box[3]), start_event,
                                         end_event)
        min_point = self.transform_point((box[0], box[2]), start_event,
                                         end_event)
    ...
    def transform_polygon(self,
                          vertices: list,
                          start_event: str = 'start',
                          end_event: str = 'end') -> list:
        return [
            self.transform_point(point, start_event, end_event)
    ...
    def shear(self,
        ...
        self._affine_transform(shear=shear_intensity,
                               fill_mode=fill_mode,
                               c_val=c_val,
    ...
    def rotate(self,
               ...
               c_val: float = 0.,
               interpolation_order: int = 1) -> None:
        self._affine_transform(theta=theta,
                               fill_mode=fill_mode,
                               c_val=c_val,
                               order=interpolation_order)
    ...
    @History.debug()
    def resize(self, target_size: tuple,
    ...
    def pad(self,
        ...
        padding = [
            x if x is not None else 0 for x in (top, bottom, left, right)
        ]
        if colour is None:
            colour = (0, 0, 0)
    ...
    def _crop(self, top: int, bottom: int, left: int, right: int) -> None:
        w, h = self.size
        self.images = [
            image[top:h - bottom, left:w - right] for image in self.images
        ]

    def _crop_inverse(self, top: int, bottom: int, left: int,
                      right: int) -> None:
        return self._pad(top, bottom, left, right, colour=(0, 0, 0))

    def _crop_point(self, point: tuple, top: int, bottom: int, left: int,
                    right: int) -> tuple:
        return point[0] - left, point[1] - top

    def _crop_point_inverse(self, point: tuple, top: int, bottom: int,
    ...
    def _pad(self, top, bottom, left, right, colour):
        self.images = [
            cv2.copyMakeBorder(image,
                               ...
                               right,
                               cv2.BORDER_CONSTANT,
                               value=colour) for image in self.images
        ]
    ...
    def _resize(self, fx: float, fy: float) -> None:
        self.images = [
            cv2.resize(image, dsize=None, fx=fx, fy=fy)
            for image in self.images
        ]
    ...
    def _resize_point_inverse(self, point: tuple, fx: float,
                              fy: float) -> tuple:
    ...
    def _affine_transform(self,
        ...
        final_affine_matrix, final_offset = _get_affine_transformation_matrix(
            self.size, theta, tx, ty, shear, zx, zy)

        self.images = [
            _apply_affine_transform(image, final_affine_matrix, final_offset,
                                    fill_mode, c_val, order)
            for image in self.images
        ]

    def _affine_transform_inverse(self,
                                  theta: float = 0,
                                  tx: float = 0,
                                  ty: float = 0,
                                  shear: float = 0,
    ...
    def _affine_transform_point(self,
                                point,
                                theta: float = 0,
                                tx: float = 0,
                                ty: float = 0,
    ...
    def _affine_transform_point_inverse(self,
                                        point,
                                        theta: float = 0,
                                        tx: float = 0,
                                        ty: float = 0,
    ...
    def _get_crop_from_slice(self, key):
        ...
        if type(key) == tuple:
            assert len(
                key) == 2, 'can only slice along columns, or columns and rows'
            if type(key[0]) == slice:
                assert (key[0].step is None) or (
                    key[0].step == 1), 'step must be 1 or None when slicing'
                top = key[0].start if key[0].start is not None else 0
                top = self.size[1] + top if top < 0 else top
                bottom = key[0].stop if key[0].stop is not None else self.size[
                    1]
                bottom = -bottom if bottom < 0 else self.size[1] - bottom
            elif key[0] == Ellipsis:
                pass
            if type(key[1]) == slice:
                assert (key[1].step is None) or (
                    key[1].step == 1), 'step must be 1 or None when slicing'
            ...
            assert (key.step is None) or (
                key.step == 1), 'step must be 1 or None when slicing'
    ...
    def _get_transform_config(self) -> list:
        return [{
            event['fn_name']: event['fn_args']
        } for event in self._transform_history]


def _transform_matrix_offset_center(matrix: np.ndarray, x: int,
                                    y: int) -> np.ndarray:
    o_x = float(x) / 2 + 0.5
    o_y = float(y) / 2 + 0.5
...
def _get_affine_transformation_matrix(size: tuple,
                                      theta: float = 0,
                                      tx: float = 0,
                                      ty: float = 0,
                                      shear: float = 0,
...
def _apply_affine_transform(x: np.ndarray,
                            final_affine_matrix: np.ndarray,
...
def _repeat_c_channels(image):
    if image.ndim == 2:
        ...
        raise ValueError('invalid image ndim {} and shape {}'.format(
